...
namespace Bingo
{
    class Program
    {
        ...
        private static void ShowChildren(string name)
        {
            GraphNode n = rg.GetNode(name);
            if (n != null)
            {
                Console.Write("{0}'s kids: ", name);
...
/*        private List<String> getChidren(string name)
        {
            GraphNode n = rg.GetNode(name);
            ...
            {
                //Console.Write("{0}'s kids: ", name);
                List<GraphEdge> friendEdges = n.GetEdges("hasChild");
                foreach (GraphEdge e in friendEdges)
                {
                    kids.Add( e.To());
                }
...
*/
        private static void ShowSiblings(string name)
        {
            ...
            foreach (GraphEdge edge in n.GetEdges())
            {
                if (edge.Label == "hasParent")
                {
                    parentname = edge.To();
                    GraphNode parent = rg.GetNode(parentname);
                    foreach (GraphEdge parentedge in parent.GetEdges())
                    {
                        if (parentedge.Label == "hasChild")
                        {
                            if (name != parentedge.To())
                            {
                                Console.WriteLine("- " + parentedge.To());
                            }
                        }
                    }
                }
        ...
        private static void ShowDescendants(string name, int level)
        {            
            if(level == -1)
            {
                Console.WriteLine(name + "'s Descendants:");
            }
            level++; 
            ...
            foreach (GraphEdge edge in n.GetEdges())
            {
                if (edge.Label == "hasChild")
                {
                    if(level==0)
                    {
                       
                        Console.WriteLine("-Child: " + edge.To());
                    }
                    if (level == 1)
                    {
                        Console.WriteLine("-GrandChild: " + edge.To());
                    }
                    if(level>= 2)
                    {
                        Console.Write("-");
                    ...
                    ShowDescendants(edge.To(), level + 1);
                }
            }
        ...
        private static void CommandLoop()
        {
            string command = "";
            string[] commandWords;
            Console.Write("Welcome to Daniel's Dutch Bingo Simulator!\n");
            ...
            while (command != "exit")
            {
                ...
                //show all orphans
                    ...
                    ShowDescendants(commandWords[1], -1);

                else if(command == "children")
                    ShowChildren(commandWords[1]);

                // illegal command
                else
                    Console.Write("\nLegal commands: read [filename], dump, descendants [personname], orphans, siblings [personname], show [personname],\n  friends [personname], exit\n");
            }
        }
